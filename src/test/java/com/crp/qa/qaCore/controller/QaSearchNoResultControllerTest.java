...
@SpringBootTest
public class QaSearchNoResultControllerTest {
...
private static final String token = "8hXa5JjiJtpq6ICoIOhXqqsAHu3bgH";
	...
	public void findPagedAll() throws Exception{
		...
		MvcResult result = mockMvc.perform(
        			get("/noresult/findPagedAll")
        			.param("token", token).param("page", "0").param("size", "20")
        		)
	...
	public void findByCreationDateBetween() throws Exception{
        ...
        @SuppressWarnings("unused")
		MvcResult result = mockMvc.perform(
        			get("/noresult/findByCreationDateBetween")
        			.param("token", token).param("page", "0").param("size", "20")
        		)
        		.andExpect(status().isOk())// 模拟向testRest发送get请求  
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))// 预期返回值的媒体类型text/plain;charset=UTF-8  
